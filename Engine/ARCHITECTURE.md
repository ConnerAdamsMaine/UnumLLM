...
- **PackedTernary**: Bit-packed storage (3 weights per 8 bits)
...
**Purpose**: Efficient model inference
...
**Purpose**: Command-line interface for end-to-end workflows
...
└── commands/
    ...
    ├── quantize.rs   # Quantization of existing models
    ├── infer.rs      # Text generation
...
**Purpose**: Python interface to core functionality

```
python/
├── lib.rs           # PyO3 module initialization
├── model.rs         # OneBitModel wrapper
├── config.rs        # ModelConfig Python binding
├── generate.rs      # Generation methods
└── tokenizer.rs     # Tokenizer wrapper
```

**Exposed APIs**:
- `ModelConfig`: Model hyperparameters
- `OneBitModel`: Main model class
